<template>
  <div @click.self="$emit('toggleCoverModal')" class="h-full w-full bg-gray-300 bg-opacity-70 fixed top-0 left-0 z-30">
    <section class="w-11/12 bg-white fixed-center p-5">
      ...
      <div class="h-32 w-64 bg-gold-500 mx-auto mb-2 flex items-center justify-center bg-white border-4 border-white overflow-hidden">
        <img v-if="propsUser.cover_url" :src="returnCoverFullPath(propsUser.cover_url)" height="128" width="256" alt="">
        <img v-else :src="dummy_header" height="128" width="256" alt="">
      </div>
      <form @submit.prevent>
